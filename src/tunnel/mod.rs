...
impl PhantomTunnel {
    ...
    pub async fn connect(&self, remote_addr: SocketAddr) -> Result<()> {
        ...
        *self.state.write() = TunnelState::Handshaking;
        ...
        let transport =
            create_transport(self.config.transport.primary, self.config.network.bind_addr, false)
                .await?;
    ...
    pub async fn listen(&self) -> Result<()> {
        *self.state.write() = TunnelState::Handshaking;
        ...
        let transport =
            create_transport(self.config.transport.primary, self.config.network.bind_addr, true)
                .await?;
